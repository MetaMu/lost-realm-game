...
(function(root){
 ...
 function capture(g){if(g.status!=='build')return null;const state={};for(const k of fields)state[k]=g[k];state.towers=g.towers.map(t=>Object.fromEntries(Object.entries(t).filter(([k,v])=>['string','number','boolean'].includes(typeof v))));return JSON.stringify({version:1,state});}
 function restore(g,raw,K){try{const data=JSON.parse(raw),s=data.state;if(data.version!==1||!s||![1,2,3].includes(s.stage)||!Number.isInteger(s.wave)||s.wave<0||s.wave>=K.stageWaves(s.stage)||!Number.isFinite(s.gold)||s.gold<0||!(s.lives>0&&s.lives<=20)||!Array.isArray(s.towers)||s.towers.length>K.PADS.length)return false;
 ...
 g.reset();for(const k of fields)if(s[k]!==undefined)g[k]=s[k];g.towers=s.towers.map(t=>({...t,...K.PADS[t.pad]}));g.status='build';return true;}catch{return false;}}
