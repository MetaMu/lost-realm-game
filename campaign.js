...
(function(root){
 ...
 Object.assign(K.TYPES,{
  lady:{name:'Lady Never Scared',title:'The Soul Gardener',power:'Healing · soul conversion',unlock:2,cost:320,color:'#afffe0',range:185,damage:0,interval:30,description:'Heals neighboring guardians for 4 HP/sec. Every 30 combat seconds, channels for 2 seconds to turn a non-boss mutant into an allied demon.'},
 ...
 class CampaignGame extends Base {
  ...
  freezeKnowME(){const targets=this.enemies.filter(e=>e.hp>0&&e.x>=0&&e.x<=K.WIDTH);if(this.status!=='wave'||this.stage!==1||this.freezeUsed||this.ultimate||!this.towers.some(t=>t.type==='knowme')||!targets.length)return false;this.freezeUsed=true;this.freezeUntil=this.time+5;for(const e of targets)e.frozenUntil=this.freezeUntil;this.emit('cast',{power:'knowme'});return true;}
  ...
  update(dt){dt=Math.max(0,Math.min(.05,dt));
   ...
   for(const c of this.conversions){c.left-=dt;if(!this.towers.includes(c.owner)||c.target.hp<=0||!this.enemies.includes(c.target)){c.left=-1;continue;}if(c.left<=0){const e=c.target;this.enemies=this.enemies.filter(a=>a!==e);this.allies.push({...e,id:this.nextId++,kind:'demon',owner:c.owner,hp:Math.min(900,e.hp),maxHp:Math.min(900,e.hp),damage:85*c.owner.level,cooldown:0,poisonUntil:0,slowUntil:0});this.emit('converted',{x:e.x,y:e.y});}}
   ...
   super.update(dt);for(const [e,speed] of engaged)e.speed=speed;
