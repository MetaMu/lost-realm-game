...
(function(root){
 ...
 class CampaignGame extends Base {
  ...
  update(dt){dt=Math.max(0,Math.min(.05,dt));
   ...
   for(const bomb of this.hellfire){bomb.left-=dt;if(bomb.left<=0&&bomb.target.hp>0){const e=bomb.target;this.hurt(e,e.maxHp*.35,null,true);this.effects.push({kind:'cannon',x:e.x,y:e.y-25,radius:80,color:'#ffb34b',life:.8,age:0});this.emit('impact',{power:'sailor'});}}this.hellfire=this.hellfire.filter(b=>b.left>0);
