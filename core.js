...
(function (root) {
  ...
  class Game {
    ...
    spawn(kind){const spec=ENEMIES[kind],hp=spec.hp*3*.9*(1+(this.wave-1)*.2)*(this.stage===3?4.5:this.stage===2?1.904:1)*(kind==='boss'?.9:1)*(kind==='boss'?(this.wave===6?.9:this.wave===12?.97:1):1);const e={...spec,bounty:spec.bounty*(this.stage===2?1.2:1)*1.25,mutated:this.stage>=2,speed:spec.speed*(this.stage>=2?1.2:1)/1.15,armor:Math.min(.65,spec.armor+(this.stage>=2?.12:0)),id:this.nextId++,kind,hp,maxHp:hp,progress:0,slowUntil:0,slow:0,poisonUntil:0,poisonDps:0,poisonOwner:null,...position(0)};this.enemies.push(e);return e;}
    ...
    fire(t,e){
      ...
      if(t.type==='host'){
        const group=new Set([t]),queue=[t];for(let q=0;q<queue.length;q++)for(const a of this.towers)if(a.type==='host'&&a.hp>0&&!group.has(a)&&distance(a,queue[q])<=190){group.add(a);queue.push(a);}const linked=group.size>1,chainLimit=linked?3+2*(group.size-1):3+(t.level===3?1:0),jumpRange=135+25*(group.size-1);const used=new Set();let current=e,prev={x:t.x,y:t.y-65};
