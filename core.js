...
(function (root) {
  ...
  class Game {
    ...
    spawn(kind){const spec=ENEMIES[kind],hp=spec.hp*3*.9*(1+(this.wave-1)*.2)*(this.stage===3?4.5:this.stage===2?3.4:1)*(kind==='boss'&&this.wave===6?.9:1);const e={...spec,mutated:this.stage>=2,speed:spec.speed*(this.stage>=2?1.2:1),armor:Math.min(.65,spec.armor+(this.stage>=2?.12:0)),id:this.nextId++,kind,hp,maxHp:hp,progress:0,slowUntil:0,slow:0,poisonUntil:0,poisonDps:0,poisonOwner:null,...position(0)};this.enemies.push(e);return e;}
