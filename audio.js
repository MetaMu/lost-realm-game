...
window.KnollAudio=class {
 ...
 play(type,phase='cast'){
  if(!['knowme','host','sailor'].includes(type)||!this.enabled||!this.ctx)return;const key=type+phase,now=this.ctx.currentTime;if(now-(this.last[key]??-10)<.25||this.active.size>=8)return;this.last[key]=now;
  if(type==='knowme'){
